...
```bash
psql "$DATABASE_URL" -f sql/player_contracts.sql # only when contract schema/helpers changed
psql "$DATABASE_URL" -f sql/readmodel_views.sql
psql "$DATABASE_URL" -f sql/readmodel_s3_export_views.sql
```

`player_contracts.sql` creates the typed contract tables, scrape status table, safe cast helpers, and contract sync procedure.
...
- `READ_MODEL_INCLUDE_PREFIXES="contracts/players/8478402.json"` publishes a standalone player contract payload.
- `READ_MODEL_INCLUDE_PREFIXES="contracts/teams/29/20212022.json"` publishes one selected-season team contract payload.
- `READ_MODEL_EXPORT_GROUPS=contracts` publishes all contract payloads, including player contracts and team-season contracts.
...
| `drafts/teams/{teamId}.json` | `GET /api/drafts/teams/:id` |
| `indexes/draft-teams.json` | Draft-by-team index (`/drafts?view=teams`) |
| `contracts/players/{playerId}.json` | Optional contract payload for `GET /api/players/:id` |
| `contracts/teams/{teamId}/{seasonId}.json` | Optional selected-season contract payload for `GET /api/teams/:id?contractsOnly=1&contractSeason=` |
...
| `indexes/player-embeddings.json` | Optional precomputed similar-player corpus (`GET /api/players/similar/:id`) |

Until `drafts/teams/{teamId}.json` and `indexes/draft-teams.json` are published, the app aggregates `drafts/{year}.json` in memory. After ETL, those dedicated objects should look like:

```json
{
  "team": { "id": 16, "abbreviation": "CHI", "name": "Chicago Blackhawks" },
  "draft": []
}
```
...
{
  "teams": [
    { "id": 16, "abbreviation": "CHI", "name": "Chicago Blackhawks", "pickCount": 412 }
  ]
}
```

Player read-model payloads may include optional salary data:
...
## Player embeddings (similar profiles)

`GET /api/players/similar/:id` vectorizes career NHL counting stats and returns the closest same-position profiles (forwards, defense, or goalies).

Current app path, no ETL change required:

1. Prefer `indexes/player-embeddings.json` when the publisher has uploaded it.
...
Publish a dedicated corpus after each ETL run so compare/player pages do not scan search buckets:
...
S3 key: `indexes/player-embeddings.json`. Include it in incremental publishes with `READ_MODEL_INCLUDE_PREFIXES="indexes/player-embeddings.json"`.

Richer later versions can add TOI, PIM, plus/minus, GAA, and save percentage from `readmodel.player_skater_stats` / `readmodel.player_goalie_stats`. If neighbor search needs to move into Postgres, store the same vectors with pgvector and keep this JSON as the CloudFront cache.
