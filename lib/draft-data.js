...
function buildTeamsFromPicks(picks, knownTeams = []) {
  ...
  for (const pick of picks || []) {
    ...
    const abbreviation =
      known?.abbreviation || existing?.abbreviation || pick.teamAbbrev || "";
    ...
    if (known?.abbreviation) existing.abbreviation = known.abbreviation;
    else if (!existing.abbreviation && abbreviation) existing.abbreviation = abbreviation;
    if (known?.name || known?.fullName) {
      existing.name = known.name || known.fullName;
    } else if (!existing.name || existing.name === existing.abbreviation) {
      existing.name = name;
    }
  }
