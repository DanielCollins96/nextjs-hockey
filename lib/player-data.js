...
export async function loadSitemapPlayers(minGames = MIN_SITEMAP_GAMES) {
  ...
  const firstBucket = await fetchReadModel(readModelPaths.playerSearch(SEARCH_BUCKETS[0]));
  if (!firstBucket) {
    const { getSitemapPlayers } = await import("./queries");
    return {
      source: "postgres",
      players: ((await getSitemapPlayers(safeMinGames)) || []).map(toSitemapPlayer),
    };
  }

  const bucketIndexes = await Promise.all([
    Promise.resolve(firstBucket),
    ...SEARCH_BUCKETS.slice(1).map((bucket) => fetchReadModel(readModelPaths.playerSearch(bucket))),
  ]);
