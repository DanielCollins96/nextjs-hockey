...
import { mergeToiIntoPlayerStats } from "./player-stats";
...
const NHL_LANDING_TIMEOUT_MS = 2500;

async function fetchNhlPlayerLanding(id) {
  if (id == null || id === "") return null;

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), NHL_LANDING_TIMEOUT_MS);

  try {
    const response = await fetch(
      `https://api-web.nhle.com/v1/player/${encodeURIComponent(id)}/landing`,
      {
    ...
    if (error.name !== "AbortError") {
      console.log("Unable to fetch NHL player landing", error);
    }
    return null;
  } finally {
...
function measurementsFromLanding(landing) {
  if (!landing) return {};
  return {
...
async function hydratePlayerMeasurements(id, playerRows, landingPromise) {
  ...
  const landing = landingPromise ? await landingPromise : await fetchNhlPlayerLanding(id);
  const measurements = measurementsFromLanding(landing);
...
async function loadPlayerBase(id, landingPromise) {
...
export async function loadPlayer(id) {
  const landingPromise = fetchNhlPlayerLanding(id);
  const base = await loadPlayerBase(id, landingPromise);
  if (base.notFound) return base;

  if (base.readModel) {
    const [{ contracts, currentContract }, landing] = await Promise.all([
      fetchPlayerContracts(id, base.readModel),
      landingPromise,
    ]);
    ...
    return {
      source: base.source,
      player: base.player,
      playerStats: mergeToiIntoPlayerStats(
        base.readModel.playerStats || base.readModel.stats || [],
        landing
      ),
  ...
  const [playerStats, awards, landing] = await Promise.all([
  ...
  return {
    source: base.source,
    player: base.player,
    playerStats: mergeToiIntoPlayerStats(playerStats, landing),
    awards,
    contracts: [],
