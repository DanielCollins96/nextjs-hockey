...
export async function getDraft(seasonId) {
  try {
    const sql = `
      ...
      LEFT JOIN newapi.skaters s ON d."playerId" = s."playerId" AND s."gameType" = 2
      LEFT JOIN newapi.goalies g ON d."playerId" = g."playerId" AND g."gameType" = 2
      WHERE d."draftYear" = $1
...
export async function getPointLeadersBySeason(season = 20222023) {
  try {
    const sql = `
      ...
      FROM newapi.players p
      LEFT JOIN newapi.season_skater s ON p."playerId" = s."playerId" AND s.season = $1 AND s."leagueAbbrev" = 'NHL'
      LEFT JOIN newapi.season_goalie g ON p."playerId" = g."playerId" AND g.season = $1 AND g."leagueAbbrev" = 'NHL'
