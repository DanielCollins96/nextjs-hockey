  ...
  try {
    const sql = `
      SELECT
          ...
          t.id AS "team.id"
      FROM newapi.players p
      JOIN newapi.season_goalie g ON p."playerId" = g."playerId" AND g.season = $1 AND g."leagueAbbrev" = 'NHL' AND g.is_active = true
      LEFT JOIN newapi.teams t ON t."fullName" = g."teamName.default"
...
export async function getGamesByDate(date) {
  try {
    const sql = `
      ...
      FROM newapi.nhl_games g
      LEFT JOIN newapi.teams ta ON g."awayTeam_abbrev" = ta."rawTricode"
      LEFT JOIN newapi.teams th ON g."homeTeam_abbrev" = th."rawTricode"
      WHERE g."gameDate" = $1
