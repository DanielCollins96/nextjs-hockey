...
export async function getTeamSkaters(id) {
    try {
        const sql = `
            SELECT DISTINCT t.id, sk."playerId",season, "gameType", sk.abbreviation, CONCAT(sk."firstName.default",' ',sk."lastName.default") as "fullName",  "gamesPlayed", "positionCode", "goals", "assists", "points", "penaltyMinutes", "plusMinus"
            FROM staging1.skaters sk
            JOIN staging1.team t ON sk.abbreviation = t.abbreviation
            WHERE t.id = $1 AND "gameType" = 2
        `
        let result = await conn.query(sql, [id])
        result = result.rows
        return result
...
export async function getTeamGoalies(id) {
    try {
        const sql = `
            SELECT t.id, g."playerId",season, "gameType",CONCAT(g."firstName.default",' ',g."lastName.default") as "fullName", "gamesPlayed", "goals", "assists", "wins", "losses", "goalsAgainstAverage", "savePercentage", "penaltyMinutes"
