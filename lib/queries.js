  ...
  try {
    const stats = `
        SELECT p."playerId",CONCAT(p."firstName", ' ', p."lastName") AS player_name,TO_CHAR(p."birthDate", 'YYYY-MM-DD') AS birthDate,p."birthCountry"
			 ...
			 ,p."sweaterNumber"
			--  ,p."currentAge"
			 ,ARRAY_AGG(d."draftYear") AS draft_seasons,ARRAY_AGG(d."overallPick") AS draft_position
...
export async function getDraft(seasonId) {
  try {
    const sql = `
    select "playerId","overallPick","pickInRound","round",CONCAT(d."firstName", ' ', d."lastName") AS "playerName","positionCode","amateurLeague","amateurClubName","teamAbbrev"
            ,"teamId"
    ...
    WHERE "draftYear" = $1
    GROUP BY "playerId","overallPick","pickInRound","round","positionCode","amateurLeague","amateurClubName","teamAbbrev","teamId", d."firstName", d."lastName"
