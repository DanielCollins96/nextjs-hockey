...
const READ_MODEL_DEBUG = process.env.READ_MODEL_DEBUG === "true";
...
export async function fetchReadModel(path, options = {}) {
  ...
  if (!url) {
    if (READ_MODEL_DEBUG) {
      console.log(
        `⚪ Read model disabled for ${path}; set READ_MODEL_BASE_URL and restart the dev server`
      );
    }
    return null;
  }
  ...
  const missStatuses = new Set(options.missStatuses || [403, 404]);
  ...
  try {
    ...
    if (missStatuses.has(response.status)) {
      if (READ_MODEL_DEBUG) console.log(`🟡 Read model MISS ${url}`);
      return null;
    }
    ...
    if (READ_MODEL_DEBUG) console.log(`🟢 Read model HIT ${url}`);
...
export const readModelPaths = {
  ...
  playerContracts: (id) => `contracts/players/${pathSegment(id)}.json`,
  teamContracts: (teamId, season) =>
    `contracts/teams/${pathSegment(teamId)}/${pathSegment(season)}.json`,
  playerSearch: (bucket) =>
    bucket
      ? `indexes/player-search/${pathSegment(bucket)}.json`
      : "indexes/player-search.json",
  playerEmbeddings: () => "indexes/player-embeddings.json",
  ...
  draftByTeam: (teamId) => `drafts/teams/${pathSegment(teamId)}.json`,
  draftYears: () => "indexes/draft-years.json",
  draftTeams: () => "indexes/draft-teams.json",
  game: (id) => `games/${pathSegment(id)}.json`,
  gameDate: (date) => `games/dates/${pathSegment(date)}.json`,
  gameDateRange: () => "indexes/game-date-range.json",
