...
export const MIN_SIMILARITY_PERCENT = 60;
...
export const SKATER_FEATURES = [
  ...
  { key: "heightInches", weight: 0.9 },
  { key: "weightPounds", weight: 0.9 },
  { key: "era", weight: 0.35 },
];
...
export const GOALIE_FEATURES = [
  ...
  { key: "heightInches", weight: 0.85 },
  { key: "weightPounds", weight: 0.85 },
  { key: "era", weight: 0.4 },
];
...
export function playerHeightInches(player) {
  const inches = toNumber(
    player?.heightInInches ?? player?.heightInches ?? player?.height_in_inches ?? player?.height
  );
  if (inches !== null && inches >= 54 && inches <= 90) return inches;

  const centimeters = toNumber(
    player?.heightInCentimeters ?? player?.heightCentimeters ?? player?.height_in_centimeters
  );
  if (centimeters !== null && centimeters >= 140 && centimeters <= 230) {
    return centimeters / 2.54;
  }
  return null;
}

export function playerWeightPounds(player) {
  const pounds = toNumber(
    player?.weightInPounds ?? player?.weightPounds ?? player?.weight_in_pounds ?? player?.weight
  );
  if (pounds !== null && pounds >= 120 && pounds <= 320) return pounds;

  const kilograms = toNumber(
    player?.weightInKilograms ?? player?.weightKilograms ?? player?.weight_in_kilograms
  );
  if (kilograms !== null && kilograms >= 55 && kilograms <= 145) {
...
export function rawPlayerFeatures(player) {
  ...
  return {
    ...
    heightInches: playerHeightInches(player),
    weightPounds: playerWeightPounds(player),
    era,
  };
}
...
function closestFeatureLabels(targetRaw, otherRaw, group, groupStats) {
  ...
  return features
    .filter((feature) => {
      if ((groupStats?.[feature.key]?.std || 0) <= 1e-6) return false;
      return Number.isFinite(targetRaw[feature.key]) && Number.isFinite(otherRaw[feature.key]);
    })
    .map((feature) => {
    ...
    .map((feature) => {
      ...
      if (feature.key === "heightInches" || feature.key === "weightPounds") return "size";
...
export function rankSimilarPlayers(index, targetPlayer, {
  ...
  excludeIds = [],
  minSimilarity = MIN_SIMILARITY_PERCENT,
} = {}) {
  ...
  const targetVector = embedPlayer(targetPlayer, index.statsByGroup[group], group);
