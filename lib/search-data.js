...
import { playerUrl, teamUrl } from "./routes";
...
function sortTeamsByMatch(left, right, compactQuery) {
  ...
  const leftRank =
    leftAbbrev === compactQuery
      ? 0
      : leftAbbrev.startsWith(compactQuery)
        ? 1
        : leftName.startsWith(compactQuery)
          ? 2
          : 3;
  const rightRank =
    rightAbbrev === compactQuery
      ? 0
      : rightAbbrev.startsWith(compactQuery)
        ? 1
        : rightName.startsWith(compactQuery)
          ? 2
          : 3;
...
export async function loadSearch(q = "", limit = "8") {
  const query = String(q).trim();
  const safeLimit = parseLimit(limit);
  const compactQuery = normalizeCompact(query);

  if (!compactQuery || compactQuery.length < 2) {
    return { source: "none", players: [], teams: [] };
  }
  ...
  if (!playerReadModel || !teams) {
    const { searchPlayers, searchTeams } = await import("./queries");
    const [dbPlayers, dbTeams] = await Promise.all([
      !playerReadModel
        ? searchPlayers(query, safeLimit)
