...
function getGamesFromReadModel(readModel) {
  if (!readModel || !Array.isArray(readModel.games)) {
    return null;
  }
...
export async function loadGames({ date, startDate, endDate } = {}) {
  ...
  if (startDate && endDate) {
    ...
    if (readModelsEnabled()) {
      ...
      const hasValidReadModels = readModels.every((readModel) => getGamesFromReadModel(readModel) !== null);

      if (hasValidReadModels) {
        games = readModels.flatMap((readModel) => getGamesFromReadModel(readModel));
    ...
    const readModel = readModelsEnabled()
      ? await fetchGameDateReadModel(formattedDate)
      : null;
    const readModelGames = getGamesFromReadModel(readModel);

    if (readModelGames !== null) {
      games = readModelGames;
      source = "s3-read-model";
    } else {
