...
export function compareQueryIds(query = {}) {
  ...
  const named = ["a", "b", "c", "d", "left", "right"].map((key) => (
...
export function mergeNhlSeasons(...statsGroups) {
  const groups = statsGroups.length === 1 && Array.isArray(statsGroups[0]) && statsGroups[0].every((item) => item == null || Array.isArray(item))
    ? statsGroups[0]
    : statsGroups;
