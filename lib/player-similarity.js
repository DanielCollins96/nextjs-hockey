...
export function mapSimilarityPlayer(player) {
  ...
  return {
    ...
    heightInInches: player?.heightInInches ?? player?.heightInches ?? null,
    weightInPounds: player?.weightInPounds ?? player?.weightPounds ?? null,
    heightInCentimeters: player?.heightInCentimeters ?? player?.heightCentimeters ?? null,
    weightInKilograms: player?.weightInKilograms ?? player?.weightKilograms ?? null,
  };
}
...
export function findTargetPlayer(corpus, id, profileOverride = null) {
  const key = String(id ?? "");
  const override = profileOverride ? mapSimilarityPlayer({ ...profileOverride, playerId: id || profileOverride.playerId }) : null;
  if (!key) return override;

  const indexed = corpus.index.byId.get(key)?.player;
  const fromCorpus = indexed || corpus.players.find((player) => String(player.playerId) === key);
  if (fromCorpus && override) {
    return {
      ...fromCorpus,
      position: fromCorpus.position || override.position,
      heightInInches: override.heightInInches ?? fromCorpus.heightInInches,
      weightInPounds: override.weightInPounds ?? fromCorpus.weightInPounds,
      heightInCentimeters: override.heightInCentimeters ?? fromCorpus.heightInCentimeters,
      weightInKilograms: override.weightInKilograms ?? fromCorpus.weightInKilograms,
    };
  }
...
export async function findSimilarPlayersSafe(id, options = {}) {
  try {
    return await findSimilarPlayers(id, options);
  } catch (error) {
    console.warn("Unable to load similar players", error?.message || error);
    return {
      source: "none",
      version: EMBEDDING_VERSION,
      playerId: id || null,
