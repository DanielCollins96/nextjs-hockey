...
async function loadCorpusFromSearchReadModels() {
  ...
  if (!hits || playersById.size === 0) return null;
...
async function loadSimilarityCorpusUncached() {
  ...
  try {
    return await loadCorpusFromPostgres();
  } catch (error) {
    console.warn("Unable to load player similarity corpus from Postgres", error?.message || error);
