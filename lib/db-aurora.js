...
function getClient() {
  if (!client) {
    console.log("Initializing RDSDataClient with region:", process.env.AURORA_REGION || "us-west-2");
    console.log(
      "AWS creds present:",
    ...
    const config = {
      region: process.env.AURORA_REGION || "us-west-2",
    };

    // Explicitly pass credentials if available (needed for Vercel serverless)
    ...
    client = new RDSDataClient(config);
  }
  return client;
}
...
if (!resourceArn || !secretArn) {
  throw new Error(
    "Aurora Data API is enabled but AURORA_CLUSTER_ARN or AURORA_SECRET_ARN is missing."
  );
}
